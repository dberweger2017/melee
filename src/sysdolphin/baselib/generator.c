    ...
    if (gen != NULL) {
        ...
        case 4:
            if (((HSD_PSCmdList*) ptclref_804D0E5C[bank][idx])->param1 ==
                    0.0F &&
                ((HSD_PSCmdList*) ptclref_804D0E5C[bank][idx])->param2 == 0.0F)
            {
                gen->aux.disc.minAngle = 0.0F;
                gen->aux.disc.maxAngle = 6.2831855F;
            } else {
                gen->aux.disc.minAngle =
